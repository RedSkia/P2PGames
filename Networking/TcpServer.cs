            ...
            try
            {
                ...
                await Task.Factory.StartNew(async () =>
                {
                    ...
                    while (this.Clients.Count < maxClients)
                    {
                        ...
                        if (!(await Authentication.Authenticate(client, token)))
                        {
                            this.OnLog?.Invoke(this, ($"TcpClient #{clientId} failed to Authenticate... @ {clientIp}", "Warn"));
                            continue;
                        }
                        this.OnLog?.Invoke(this, ($"TcpClient #{clientId} Authenticated!!! @ {clientIp}", "Advert"));
